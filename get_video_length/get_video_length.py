...
class gvl:
    ...
    def _check_offline_video_id_validity(self,url:str) -> bool:
        ...
        return url.startswith("https://") and len(url.rsplit("v=")[1]) == 11 or len(url.rsplit("/")) == 11
    ...
    def _check_online_video_id_validity(self,url:str) -> bool:
        ...
        """
        checker_url = f"https://img.youtube.com/vi/{self._get_url_id(url)}/mqdefault.jpg"
        r = requests.get(checker_url)
        invalid_image_dimensions = (120,90)
        #print(self._get_image_dims(checker_url))
        return True if self._get_image_dims(checker_url) != invalid_image_dimensions else False
    ...
    def video_file(self,filename:str) -> tuple[float, int, float]:
        ...
        if fps <= 0 or frame_count <= 0:
            raise ZeroDivisionError("either FPS or FRAME_COUNT is 0, are you sure it's a video file?")
